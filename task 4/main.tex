...
\section*{Задача 3} Постройте по НКА $\mathcal{A}$ из предыдущей задачи эквивалентный ДКА $\mathcal{B}$ по алгоритму НКА --- ДКА.
...
\section*{Задача 4} $L$ -- конечный язык. Выполняется для него лемма о накачке?
